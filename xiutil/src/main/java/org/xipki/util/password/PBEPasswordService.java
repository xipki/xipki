...
public class PBEPasswordService {
  ...
  public static char[] decryptPassword(
    ...
    // 4 = "PBE:".length()
    String passwordHintWithoutPrefix = passwordHint.substring(4);
  ...
  public static String encryptPassword(
      PBEAlgo algo, int iterationCount, char[] masterPassword, char[] password)
      throws PasswordResolverException {
    if (iterationCount < 1 || iterationCount > 65535) {
      throw new IllegalArgumentException("iterationCount may not be out of " +
          "the range [1, 65535]: " + iterationCount);
    }
