...
public class IoUtil {
  ...
  public static boolean deleteDir(File dir, boolean prependBaseDir) {
    ...
    try {
      ...
      LOG.error("error deleting dir {}", dir);
      if (LOG.isDebugEnabled()) {
        LOG.debug("error deleting dir {}", dir, e);
      }
      return false;
    }
  }
  ...
  public static String expandFilepath(String path, boolean prependBaseDir) {
    ...
    if (prependBaseDir) {
      return XipkiBaseDir.basedir() == null ? path
              : Paths.get(XipkiBaseDir.basedir(), path).toString();
    } else {
      return path;
    }
  }
  ...
  public static String base64Encode(byte[] data, boolean withLineBreak) {
    return Base64.getEncoder().encodeToString(data, withLineBreak);
  }
