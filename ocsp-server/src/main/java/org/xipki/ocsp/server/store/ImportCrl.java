...
class ImportCrl {
  ...
  private static class CrlDirInfo {
    ...
    private final Validity crlTimeoutDuration;

    CrlDirInfo(int crlId, String crlName, File crlDir, boolean updateMe, boolean deleteMe,
               boolean crlDownloaded, File updatemeFile, File crlFile, Validity crlTimeoutDuration) {
      this.crlId = crlId;
      ...
      this.crlTimeoutDuration = crlTimeoutDuration;
    }
  ...
  public boolean importCrlToOcspDb() {
    ...
    for (File crlDir : crlDirs) {
      ...
      boolean crlDownloaded = false;
      Validity crlTimeoutDuration = null;
      File crlDownloadFile = new File(crlDir, "crl.download");
      if (crlDownloadFile.exists()) {
        crlDownloaded = true;
        try {
          Properties downloadProps = CrlDbCertStatusStore.loadProperties(crlDownloadFile);
          if (downloadProps.containsKey("crl.import.timeout")) {
            crlTimeoutDuration = Validity.getInstance(downloadProps.getProperty("crl.import.timeout"));
          }
        } catch (IOException ex) {
          LOG.error("error reading " + crlDownloadFile.getPath(), ex);
        }
      }
      ...
      CrlDirInfo crlDirInfo = new CrlDirInfo(crlId, crlName, crlDir, true, deleteMe,
              crlDownloaded, updatemeFile, crlFile, crlTimeoutDuration);
    ...
    try {
      ...
      for (CrlDirInfo crlDirInfo : crlDirInfos) {
        if (crlDirInfo.updateMe) {
          ImportCrlRunnable importCrlRunnable = new ImportCrlRunnable(conn, crlDirInfo, maxId);
          threadPoolExecutor.submit(importCrlRunnable);
          if (crlDirInfo.crlTimeoutDuration != null && crlDirInfo.crlTimeoutDuration.approxMinutes() > timeoutMinutes) {
            timeoutMinutes = crlDirInfo.crlTimeoutDuration.approxMinutes();
          }
        }
      }
      if (timeoutMinutes == 0) {
      ...
      return threadPoolExecutor.awaitTermination(timeoutMinutes, TimeUnit.MINUTES);
    } catch (Throwable th) {
  ...
  private class ImportCrlRunnable implements Runnable {

    private Connection conn;

    private CrlDirInfo crlDirInfo;

    private AtomicLong maxId;

    public ImportCrlRunnable(Connection conn, CrlDirInfo crlDirInfo, AtomicLong maxId) {
      this.conn = conn;
      this.crlDirInfo = crlDirInfo;
      this.maxId = maxId;
    }

    @Override
    public void run() {
      importCrl(conn, crlDirInfo, maxId);
    }
    private void importCrl(Connection conn, CrlDirInfo crlDirInfo, AtomicLong maxId) {
      ...
      try {
        ...
        if (crl == null) {
          ...
          importCrlRevokedCertificates(conn, id, caCert, crl, crlDir, startTimeSec, maxId);
          commit(conn);
    ...
    private void importCrlRevokedCertificates(
            Connection conn, int crlInfoId, CertWrapper caCert, CrlStreamParser crl, File crlDir, long startTimeSec, AtomicLong maxId)
            throws DataAccessException, ImportCrlException, IOException {
      long timeMillis = System.currentTimeMillis();
      LOG.error("IMPORTING REVOKED CERTS");
      ...
      if (serialNumbersFiles != null && serialNumbersFiles.length > 0) {
        ...
        for (File serialNumbersFile : serialNumbersFiles) {
          ...
          if (num >= sqlBatchCommit) {
            ...
            commit(conn);
            LOG.error("COMMIT TIME 1: "+(System.currentTimeMillis()-time));
          }
        }

        long time = System.currentTimeMillis();
        commit(conn);
        LOG.error("COMMIT TIME 2: "+(System.currentTimeMillis()-time));
      }
      LOG.error("FINISHED IMPORTING REVOKED CERTS: "+(System.currentTimeMillis()-timeMillis));
  ...
  private synchronized void commit(Connection conn) throws DataAccessException {
      try {
        conn.commit();
      } catch (SQLException ex) {
