...
public class CtLog {
    ...
    public static SerializedSCT getInstance(byte[] encoded) {
      ...
      while (offsetObj.get() < encoded.length) {
        ...
        SignedCertificateTimestamp sct =
            SignedCertificateTimestamp.getInstance(encoded, offsetObj, sctLen);
        scts.add(sct);
      }
  ...
  public static class SignatureAndHashAlgorithm {
    ...
    public static SignatureAndHashAlgorithm getInstance(byte[] encoded) {
      return new SignatureAndHashAlgorithm(HashAlgorithm.ofCode(encoded[0]),
  ...
  public static class SignedCertificateTimestamp {
    ...
    public static SignedCertificateTimestamp getInstance(
        byte[] encoded, AtomicInteger offsetObj, int len) {
      ...
      byte[] extensions = (extensionsLen == 0) ? new byte[0]
          : copyOf(encoded, offset, extensionsLen);
    ...
    public SignedCertificateTimestamp(
        byte version, byte[] logId, long timestamp, byte[] extensions,
        DigitallySigned digitallySigned) {
    ...
    public byte[] getEncoded() {
      ...
      //41: 1 + 32 + 8, 2:  length of extensions
      int totalLen = 41 + 2 + extensions.length + encodedDs.length;
  ...
  public static class AddPreChainRequest {
    ...
    public AddPreChainRequest(List<byte[]> chain) {
  ...
  public static class AddPreChainResponse {
    ...
    public static AddPreChainResponse parse(JsonMap json)
      ...
      return new AddPreChainResponse((byte) sct_version,
          json.getNnBytes("id"), json.getNnLong("timestamp"),
          json.getBytes("extensions"), json.getNnBytes("signature"));
    }
