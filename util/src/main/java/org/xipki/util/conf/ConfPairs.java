...
public class ConfPairs implements JsonEncodable {
  ...
  private static class Unmodifiable extends ConfPairs {
    ...
    @Override
    public String removePair(String name) {
  ...
  public ConfPairs(String confPairs) {
    if (confPairs == null || confPairs.isEmpty()) {
      return;
    }
  ...
  public String removePair(String name) {
    return pairs.remove(Args.notBlank(name, "name"));
  }
  ...
  public JsonMap toCodec() {
    ...
    for (Map.Entry<String, String> v : pairs.entrySet()) {
      String pairKey = v.getKey();
  ...
  public String toStringOmitSensitive(
      Collection<String> nameKeywords, Collection<String> ignoreList) {
  ...
  public boolean equals(Object obj) {
    ...
    for (String name : thisNames) {
      if (!Objects.equals(value(name), other.value(name))) {
        return false;
      }
    }
  ...
  public static ConfPairs parse(JsonMap json) throws CodecException {
    if (json == null) {
      return null;
    }
