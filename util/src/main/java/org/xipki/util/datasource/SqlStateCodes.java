...
public class SqlStateCodes {
  ...
  private SqlStateCodes() {
    badSqlGrammarCodes = toSet(
        BGE_DYNAMIC_SQL_ERROR,
        BGE_CARDINALITY_VIOLATION,
        BGE_SYNTAX_ERROR_DIRECT_SQL,
        BGE_SYNTAX_ERROR_DYNAMIC_SQL,
        BGE_GENERAL_SQL_SYNTAX_ERROR);
    dataIntegrityViolationCodes = toSet(
        DIV_DATA_TRUNCATION,
        DIV_INTEGRITY_CONSTRAINT_VIOLATION,
        DIV_NO_DATA_FOUND,
        DIV_TRIGGERED_DATA_CHANGE_VIOLATION,
        DIV_VALUE_OUTOF_RANGE,
        DIV_WITH_CHECK_VIOLATION);
