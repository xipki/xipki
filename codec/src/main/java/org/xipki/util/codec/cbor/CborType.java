...
public class CborType {
   ...
   *
   * @param mt the major type to return as string, values from [0..7] are
   *           supported.
   * @return the name of the given major type, as String, never
   *          <code>null</code>.
   * @throws IllegalArgumentException in case the given major type is not
   *         supported.
   */
  ...
  /**
   * @return the additional information of this type, as integer value from
   *         [0..31].
   */
  ...
  public boolean isBooleanType() {
    return m_major == TYPE_FLOAT_SIMPLE && (m_additional == TRUE || m_additional == FALSE);
  }
  ...
  public boolean isInt() {
    return m_major == TYPE_UNSIGNED_INTEGER || m_major == TYPE_NEGATIVE_INTEGER;
  }
  ...
  public boolean isNull() {
    return m_major == TYPE_FLOAT_SIMPLE && m_additional == NULL;
  }

  public boolean isUndefined() {
    return m_major == TYPE_FLOAT_SIMPLE && m_additional == UNDEFINED;
  }

  public boolean isNullThenRead(CborDecoder decoder) throws CodecException {
    boolean isNull = (m_major == TYPE_FLOAT_SIMPLE && m_additional == NULL);
    if (isNull) {
        decoder.readNull();
    }
    return isNull;
  ...
  /**
   * @return <code>true</code> if this type allows for an infinite-length
   *         payload,
   *         <code>false</code> if only definite-length payloads are allowed.
   */
  public boolean isBreakAllowed() {
    return m_major == TYPE_ARRAY || m_major == TYPE_BYTE_STRING
        || m_major == TYPE_MAP   || m_major == TYPE_TEXT_STRING;
  }

  /**
   * Determines whether the major type of the given {@link CborType} equals the
   * major type of this {@link CborType}.
   *
   * @param other the {@link CborType} to compare against, cannot be
   *              <code>null</code>.
   * @return <code>true</code> if the given {@link CborType} is of the same
   *         major type as this {@link CborType}, <code>false</code> otherwise.
   * @throws IllegalArgumentException in case the given argument was
   *         <code>null</code>.
   */
  ...
  /**
   * Determines whether the major type of the given byte value (representing an
   * encoded {@link CborType}) equals the major type of this {@link CborType}.
   *
   * @param encoded the encoded CBOR type to compare.
   * @return <code>true</code> if the given byte value represents the same
   *         major type as this {@link CborType}, <code>false</code> otherwise.
   */
