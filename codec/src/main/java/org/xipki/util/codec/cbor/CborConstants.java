...
public interface CborConstants {
  ...
  /** The CBOR-encoded boolean <code>false</code> value
   * (encoded as "simple value": {@link #TYPE_FLOAT_SIMPLE}). */
  int FALSE = 0x14;

  /** The CBOR-encoded boolean <code>true</code> value
   * (encoded as "simple value": {@link #TYPE_FLOAT_SIMPLE}). */
  int TRUE = 0x15;

  /** The CBOR-encoded <code>null</code> value
   * (encoded as "simple value": {@link #TYPE_FLOAT_SIMPLE}). */
  int NULL = 0x16;
   ...
   * see {@link #TYPE_FLOAT_SIMPLE}). */
  int HALF_PRECISION_FLOAT = TWO_BYTES;

  /** Denotes a single-precision float (four-byte IEEE 754,
   * see {@link #TYPE_FLOAT_SIMPLE}). */
  int SINGLE_PRECISION_FLOAT = FOUR_BYTES;

  /** Denotes a double-precision float (eight-byte IEEE 754,
   * see {@link #TYPE_FLOAT_SIMPLE}). */
  int DOUBLE_PRECISION_FLOAT = EIGHT_BYTES;

  /** The CBOR-encoded "break" stop code for unlimited arrays/maps. */
  int BREAK = 0x1f;

  /** Semantic tag value describing date/time values in the standard
   * format (UTF8 string, RFC3339). */
  int TAG_STANDARD_DATE_TIME = 0;

  /** Semantic tag value describing date/time values as Epoch timestamp
   * (numeric, RFC3339). */
  int TAG_EPOCH_DATE_TIME = 1;

  /** Semantic tag value describing a positive big integer value
   * (byte string). */
  int TAG_POSITIVE_BIGINT = 2;

  /** Semantic tag value describing a negative big integer value
   * (byte string). */
  int TAG_NEGATIVE_BIGINT = 3;

  /** Semantic tag value describing a decimal fraction value
   * (two-element array, base 10). */
  int TAG_DECIMAL_FRACTION = 4;

  /** Semantic tag value describing a big decimal value
   * (two-element array, base 2). */
  int TAG_BIGDECIMAL = 5;

  /** Semantic tag value describing an expected conversion to base64url
   * encoding. */
  int TAG_EXPECTED_BASE64_URL_ENCODED = 21;

  /** Semantic tag value describing an expected conversion to base64
   * encoding. */
  int TAG_EXPECTED_BASE64_ENCODED = 22;

  /** Semantic tag value describing an expected conversion to base16
   * encoding. */
  ...
  /** Semantic tag value describing a regular expression string
   * (UTF8 string, PCRE). */
  int TAG_REGEXP = 35;
