...
public class OcspServletFilter implements Filter {
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    HealthCheckServlet0 healthUnderlying = new HealthCheckServlet0();
    healthUnderlying.setServer(this.server);
    this.healthServlet = new HealthCheckServlet();
    this.healthServlet.setUnderlying(healthUnderlying);

    HttpOcspServlet0 underlying = new HttpOcspServlet0();
    underlying.setServer(this.server);
    underlying.setLogReqResp(logReqResp);

    this.ocspServlet = new HttpOcspServlet();
    this.ocspServlet.setUnderlying(underlying);
    ...
    if (remoteMgmtEnabled) {
      if (CollectionUtil.isNotEmpty(remoteMgmt.getCerts())) {
        ...
        if (CollectionUtil.isEmpty(certs)) {
          ...
          HttpMgmtServlet0 mgmtUnderlying = new HttpMgmtServlet0();
          mgmtUnderlying.setMgmtCerts(CollectionUtil.listToSet(certs));
          mgmtUnderlying.setOcspServer(server);

          mgmtServlet = new HttpMgmtServlet();
          mgmtServlet.setUnderlying(mgmtUnderlying);
        }
      }
    }
  } // method init
