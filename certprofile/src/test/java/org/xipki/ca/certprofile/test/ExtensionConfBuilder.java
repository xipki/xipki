...
public class ExtensionConfBuilder {
  ...
  public static ExtensionValueConf.KeyUsage createKeyUsage(
    ...
    List<ExtensionValueConf.SingleKeyUsages> list = new ArrayList<>();

    if (!signOnlyKeySpecs.isEmpty()) {
      list.add(new ExtensionValueConf.SingleKeyUsages(signOnlyKeySpecs,
          reqSignUsages, optSignUsages));
    }

    if (!encOnlyKeySpecs.isEmpty()) {
      list.add(new ExtensionValueConf.SingleKeyUsages(encOnlyKeySpecs, reqEncUsages, optEncUsages));
    }
    ...
    if (!signEncKeySpecs.isEmpty()) {
      ...
      list.add(new ExtensionValueConf.SingleKeyUsages(signEncKeySpecs, reqUsages, optUsages));
    }
    ...
    if (!encKaKeySpecs.isEmpty()) {
      ...
      list.add(new ExtensionValueConf.SingleKeyUsages(encKaKeySpecs, reqUsages, optUsages));
    }
    ...
    if (!allKeySpecs.isEmpty()) {
      ...
      list.add(new ExtensionValueConf.SingleKeyUsages(allKeySpecs, reqUsages, optUsages));
    }

    // merge entries with same key-usages
    int size = list.size();
    if (size > 1) {
      List<ExtensionValueConf.SingleKeyUsages> newList = new ArrayList<>(size);
      newList.add(list.get(0));
      for (int i = 1; i < size; i++) {
        ExtensionValueConf.SingleKeyUsages u = list.get(i);
        ExtensionValueConf.SingleKeyUsages nu = null;
        for (ExtensionValueConf.SingleKeyUsages v : newList) {
          if (u.required().equals(v.required()) && u.optional().equals(v.optional())) {
            nu = v;
            break;
          }
        }
    ...
    for (ExtensionValueConf.SingleKeyUsages v : list) {
    ...
    return new ExtensionValueConf.KeyUsage(list);
  ...
  public static ExtensionValueConf.AuthorityInfoAccess createAuthorityInfoAccess() {
    return new ExtensionValueConf.AuthorityInfoAccess(true, true);
  } // method createAuthorityInfoAccess

  public static ExtensionValueConf.BasicConstraints createBasicConstraints(int pathLen) {
    return new ExtensionValueConf.BasicConstraints(pathLen);
  }

  public static ExtensionValueConf.ExtendedKeyUsage createExtendedKeyUsage(
      ExtendedKeyUsageID[] requiredUsages, ExtendedKeyUsageID[] optionalUsages) {
    return new ExtensionValueConf.ExtendedKeyUsage(
  ...
  public static ExtensionValueConf.CertificatePolicies createCertificatePolicies(
      Map<CertificatePolicyID, String> policies) {
    if (policies == null || policies.isEmpty()) {
      return null;
    }

    List<ExtensionValueConf.CertificatePolicyInformationType> pis
        = new ArrayList<>(policies.size());
    for (CertificatePolicyID oid : policies.keySet()) {
      List<ExtensionValueConf.PolicyQualifier> qualifiers = null;
      String cpsUri = policies.get(oid);
      if (cpsUri != null) {
        ExtensionValueConf.PolicyQualifier qualifier = new ExtensionValueConf.PolicyQualifier(
                ExtensionValueConf.PolicyQualifierType.cpsUri, cpsUri);
        qualifiers = List.of(qualifier);
      }

      ExtensionValueConf.CertificatePolicyInformationType single =
          new ExtensionValueConf.CertificatePolicyInformationType(oid, qualifiers);
      pis.add(single);
    }

    return new ExtensionValueConf.CertificatePolicies(pis);
  ...
  public static ExtensionValueConf.PolicyConstraints createPolicyConstraints(
    ...
    return new ExtensionValueConf.PolicyConstraints(
  ...
  public static ExtensionValueConf.InhibitAnyPolicy createInhibitAnyPolicy(int skipCerts) {
    return new ExtensionValueConf.InhibitAnyPolicy(skipCerts);
  ...
  public static ExtensionValueConf.TlsFeature createTlsFeature(TlsExtensionType... features) {
    ...
    return new ExtensionValueConf.TlsFeature(featureCodes);
  } // method createTlsFeature

  public static ExtensionValueConf.SmimeCapabilities createSmimeCapabilities() {
    // DES-EDE3-CBC
    ExtensionValueConf.SmimeCapability cap1 =
        new ExtensionValueConf.SmimeCapability(OIDs.Algo.id_DES_EDE3_CBC, null);

    // RC2-CBC keysize 128
    ExtensionValueConf.SmimeCapability cap2 =
        new ExtensionValueConf.SmimeCapability(OIDs.Algo.id_RC2_CBC, 128);

    // RC2-CBC keysize 64
    ExtensionValueConf.SmimeCapability cap3 =
        new ExtensionValueConf.SmimeCapability(OIDs.Algo.id_RC2_CBC, 64);

    return new ExtensionValueConf.SmimeCapabilities(Arrays.asList(cap1, cap2, cap3));
