...
import org.xipki.ca.certprofile.xijson.conf.ExtensionValueConf.SmimeCapabilities;
...
public class V1SmimeCapabilities {
  ...
  public SmimeCapabilities toV2() {
    List<ExtensionValueConf.SmimeCapability> list = new ArrayList<>(capabilities.size());
    for (SmimeCapability c : capabilities) {
      Integer v2Parameter = null;
      if (c.parameter != null) {
        if (c.parameter.binary != null) {
          try {
            ASN1Integer ai = ASN1Integer.getInstance(c.parameter.binary.value());
          ...
          try {
            ...
            LOG.warn("SmimeCapability.parameter.integer not an int: {}", c.parameter.integer);
          }
        }
      }

      list.add(new ExtensionValueConf.SmimeCapability(c.capabilityId.oid(), v2Parameter));
    }
  ...
  private static class SmimeCapability {
    ...
    public SmimeCapability(DescribableOid capabilityId, SmimeCapabilityParameter parameter) {
    ...
    public static SmimeCapability parse(JsonMap json) throws CodecException {
      ...
      return new SmimeCapability(DescribableOid.parseNn(json, "capabilityId"), parameter);
    }
  ...
  private static class SmimeCapabilityParameter {
    ...
    public SmimeCapabilityParameter(Integer integer, DescribableBinary binary) {
    ...
    public static SmimeCapabilityParameter parse(JsonMap json)
      ...
      return new SmimeCapabilityParameter(json.getInt("integer"), binary);
    }
