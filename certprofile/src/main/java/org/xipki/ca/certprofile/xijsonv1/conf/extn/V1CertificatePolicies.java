...
import org.xipki.ca.certprofile.xijson.conf.ExtensionValueConf.CertificatePolicies;
...
public class V1CertificatePolicies {
  ...
  private static class CertificatePolicyInformationType {
    ...
    private final List<ExtensionValueConf.PolicyQualifier> policyQualifiers;

    private CertificatePolicyInformationType(
        DescribableOid policyIdentifier,
        List<ExtensionValueConf.PolicyQualifier> policyQualifiers) {
    ...
    public static CertificatePolicyInformationType parse(JsonMap json) throws CodecException {
      ...
      List<ExtensionValueConf.PolicyQualifier> policyQualifiers = null;
      if (list != null) {
        policyQualifiers = new ArrayList<>(list.size());
        for (JsonMap v : list.toMapList()) {
          policyQualifiers.add(ExtensionValueConf.PolicyQualifier.parse(v));
        }
      }
      return new CertificatePolicyInformationType(
          DescribableOid.parseNn(json, "policyIdentifier"), policyQualifiers);
    }
  ...
  public static V1CertificatePolicies parse(JsonMap json) throws CodecException {
  ...
  public CertificatePolicies toV2() {
    List<ExtensionValueConf.CertificatePolicyInformationType> list =
        new ArrayList<>(certificatePolicyInformations.size());

    for (CertificatePolicyInformationType info : certificatePolicyInformations) {
      list.add(new ExtensionValueConf.CertificatePolicyInformationType(
              CertificatePolicyID.ofOid(info.policyIdentifier.oid()), info.policyQualifiers));
    }
