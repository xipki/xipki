...
public class V1SubjectKeyIdentifierControl {
  ...
  public SubjectKeyIdentifierControl toV2() {
    ...
    if (truncateMethod != null) {
      ...
      } else if (truncateMethod.startsWith("R") || truncateMethod.startsWith("r")) {
        v2TruncateMethod = SubjectKeyIdentifierControl.TruncateMethod.RIGHT;
      } else {
        throw new IllegalArgumentException("unsupported truncateMethod '" + truncateMethod + "'");
      }

      truncateByteSize = Integer.parseInt(truncateMethod.substring(1));
    }

    return new SubjectKeyIdentifierControl(method, hashAlgo, v2TruncateMethod, truncateByteSize);
  }
