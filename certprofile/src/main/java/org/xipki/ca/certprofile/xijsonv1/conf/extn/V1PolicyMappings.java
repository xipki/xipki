...
import org.xipki.ca.certprofile.xijson.conf.ExtensionValueConf.PolicyMappings;
...
public class V1PolicyMappings {
  ...
  public PolicyMappings toV2() {
    List<ExtensionValueConf.PolicyIdMappingType> list = new ArrayList<>(mappings.size());
    for (PolicyIdMappingType t : mappings) {
      CertificatePolicyID issuerDomainPolicy = null;
      if (t.issuerDomainPolicy != null) {
        issuerDomainPolicy = CertificatePolicyID.ofOid(t.issuerDomainPolicy.oid());
      }

      CertificatePolicyID subjectDomainPolicy = null;
      if (t.subjectDomainPolicy != null) {
        subjectDomainPolicy = CertificatePolicyID.ofOid(t.subjectDomainPolicy.oid());
      }

      list.add(new ExtensionValueConf.PolicyIdMappingType(issuerDomainPolicy, subjectDomainPolicy));
    }
