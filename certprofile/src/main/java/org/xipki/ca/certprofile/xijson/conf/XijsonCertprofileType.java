...
public class XijsonCertprofileType implements JsonEncodable {
  ...
  public static XijsonCertprofileType parse(byte[] confBytes) throws CertprofileException {
  ...
  public static XijsonCertprofileType parse(File confFile) throws CertprofileException {
  ...
  public void setSignatureAlgorithms(List<SignAlgo> signatureAlgorithms) {
    ...
    this.signatureAlgorithms = (signatureAlgorithms == null) ? null
        : new ArrayList<>(new HashSet<>(signatureAlgorithms));
  }
  ...
  public void setKeyAlgorithms(List<KeySpec> keyAlgorithms) {
    // remove duplicated algorithms
    this.keyAlgorithms = (keyAlgorithms == null) ? null
        : new ArrayList<>(new HashSet<>(keyAlgorithms));
  }
  ...
  public ExtensionsControl buildExtensionControls() throws CertprofileException {
    // Extension controls
    ...
    for (ExtensionType extn : extensions()) {
      ...
      controls.add(new ExtensionControl(oid, extn.isCritical(), extn.isRequired(), inReq));
    }
  ...
  public JsonMap toCodec() {
    ...
    JsonMap ret = new JsonMap().put("metadata",  metadataMap)
        ...
        .putEnum("certDomain", (certDomain == CertDomain.RFC5280) ? null : certDomain)
        .put("maxSize", maxSize)
    ...
    if (signatureAlgorithms != null) {
      ...
      for (SignAlgo v : signatureAlgorithms) {
        texts.add(v.name().toUpperCase().replace('_', '-'));
      }
      ret.putStrings("signatureAlgorithms", texts, true);
    }
    ...
    if (keyAlgorithms != null) {
      ...
      ret.putStrings("keyAlgorithms", texts, true);
    }
  ...
  public static List<SignAlgo> ofSignAlgos(List<String> names) throws NoSuchAlgorithmException {
  ...
  private static SignAlgo ofSignAlgo(String name) throws NoSuchAlgorithmException {
    name = name.trim();
