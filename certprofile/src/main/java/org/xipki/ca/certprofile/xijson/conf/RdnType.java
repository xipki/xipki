...
public class RdnType implements JsonEncodable {
  ...
  public RdnType(AttributeType type, String value, Integer minOccurs, Integer maxOccurs) {
    ...
    if (value != null) {
      if (this.minOccurs != 1 || this.maxOccurs != 1) {
        throw new IllegalArgumentException("(minOccurs, maxOccurs) is not (1,1), " +
  ...
  public void setToSAN(GeneralNameTag toSAN) {
    ...
    if (this.minOccurs > 1) {
      throw new IllegalArgumentException("minOccurs shall be 1, but is " + this.minOccurs);
    }
    this.toSAN = toSAN;
  }
  ...
  public RdnControl toRdnControl() {
    RdnControl ret = (value == null)
        ? new RdnControl(type.oid(), minOccurs, maxOccurs)
