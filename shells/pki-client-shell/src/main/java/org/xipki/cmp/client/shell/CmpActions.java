...
public class CmpActions {
  ...
  public abstract static class AuthClientAction extends ClientAction {
    ...
    @Option(name = "--signer-password", description =
        "Signer password, as plaintext or PBE-encrypted.")
    private String signerPasswordHint;

    protected Requestor getRequestor()
        throws IllegalCmdParamException, ObjectCreationException,
        IOException, PasswordResolverException {
      ...
      if (signerP12File != null) {
        ...
        if (signerP12SigAlgo != null) {
          try {
            sc.setAlgo(SignAlgo.getInstance(signerP12SigAlgo));
        ...
        if (signerPasswordHint == null) {
          signerPasswordHint = new String(readPassword(
        ...
        return new Requestor.PbmMacCmpRequestor(
            ...
            iterationCount, SignAlgo.HMAC_SHA256);
      }
    }
  ...
  public static class CmpCacert extends ClientAction {
    ...
    @Option(name = "--out", aliases = "-o", required = true, description =
        "where to save the CA certificate")
    @Completion(FileCompleter.class)
  ...
  public static class CmpCacertchain extends ClientAction {
    ...
    @Option(name = "--out", aliases = "-o", required = true, description =
        "where to save the CA certificate chain")
    @Completion(FileCompleter.class)
    private String outFile;
    ...
    protected Object execute0() throws Exception {
      ...
      try {
        ...
        throw new CmdFailure("Error while retrieving CA certificate chain: "
            + ex.getMessage(), ex);
      }
