...
public class OcspActions {
  ...
  public abstract static class BaseOcspStatusAction
    ...
    @Option(name = "--cert", aliases = "-c", multiValued = true, description =
        "certificate files")
    @Completion(FileCompleter.class)
     ...
     * @param encodedCerts
     *        Map of serial number and the corresponding certificate.
     *        Could be {@code null}.
     * @throws Exception
     *         if checking failed.
     */
     ...
     * numbers and for the given encoded certificates.
     *
     * @param response
          ...
          if (isBlank(serverUrl)) {
            if (CollectionUtil.isEmpty(ocspUrls)) {
              ...
              if (ocspUrl != null && !ocspUrl.equals(url)) {
                throw new IllegalCmdParamException("given certificates have " +
                    "different OCSP responder URL in certificate");
              } else {
                ocspUrl = url;
              }
            }
  ...
  public abstract static class CommonOcspStatusAction extends XiAction {
    ...
    @Option(name = "--issuer", aliases = "-i", required = true, description =
    ...
    @Option(name = "--allow-no-nonce-in-resp", description =
        "allow response without nonce, only applied if request has nonce.")
    ...
    @Option(name = "--sig-alg", multiValued = true, description =
        "comma-separated preferred signature algorithms")
  ...
  public static class OcspStatus extends BaseOcspStatusAction {
    ...
    protected void processResponse(
      ...
      boolean extendedRevoke =
          basicResp.getExtension(OIDs.Extn.id_pkix_ocsp_extendedRevoke) != null;
      ...
      if (null == basicResp.getSignature()) {
        ...
        if (respIssuer != null) {
          ...
          if (!certValid) {
            throw new CmdFailure("response is equipped with valid signature " +
                "but the OCSP signer is not trusted");
          }
        } else {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (singleCertStatus == null) {
          ...
          Extension ext = singleResp.getExtension(OIDs.Extn.invalidityDate);
          Date invTime = (ext == null) ? null
              : ASN1GeneralizedTime.getInstance(ext.getParsedValue()).getDate();
        ...
        boolean issuerMatch = issuerHash.match(hashAlgo,
