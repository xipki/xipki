...
public class CertActions {
  ...
  public static class EnrollCert extends CaAction {
    ...
    @Option(name = "--p12-password", description = "Password for the PKCS#12 file")
    ...
    protected Object execute0()
      ...
      if (StringUtil.isNotBlank(csrFile)) {
        ...
        char[] p12Pwd;
        if (StringUtil.isNotBlank(p12Password)) {
          p12Pwd = p12Password.toCharArray();
        } else {
          p12Pwd = readPassword("Enter the password for the PKCS#12 keystore!");
        }
        ...
        p12Ks.load(null, p12Pwd);
        p12Ks.setKeyEntry("main", privateKey, p12Pwd, new Certificate[]{cert});

        byte[] ksBytes;
        try (ByteArrayOutputStream os = new ByteArrayOutputStream()) {
          p12Ks.store(os, p12Pwd);
          ksBytes = os.toByteArray();
        }

        String ksFileName = outFile.substring(0, outFile.lastIndexOf('.')) + ".p12";
        saveVerbose("saved PKCS#12 keystore to file", ksFileName, ksBytes);
      }
