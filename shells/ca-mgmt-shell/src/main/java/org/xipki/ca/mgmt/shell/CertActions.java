...
public class CertActions {
  ...
  public static class EnrollCert extends CaAction {
    ...
    @Option(name = "--key-password", description =
        "Password to protect the private key, as plaintext or PBE-encrypted.\n" +
    ...
    protected Object execute0() throws Exception {
      ...
      if (StringUtil.isNotBlank(csrFile)) {
        ...
        char[] keyPwd = null;
        if (needKeyPwd) {
          keyPwd = readPasswordIfNotSet("Enter password to protect the private key",
              keyPasswordHint);
        }
        ...
        PrivateKeyInfo privKeyInfo = PrivateKeyInfo.getInstance(keyCertBytesPair.key());
        ...
        if (StringUtil.orEqualsIgnoreCase(keyOutform, "p12", "pkcs12")) {
          ...
          if (keyPwd == null) {
            saveVerbose("save unencrypted key to file", ksFilePrefix + "-key.pem",
                PemEncoder.encode(keyCertBytesPair.key(), PemEncoder.PemLabel.PRIVATE_KEY));
          } else {
            ...
            PrivateKey privKey = KeyUtil.getPrivateKey(privKeyInfo);
  ...
  public static class GetCrl extends CrlAction {
    ...
    @Override
    protected Object execute0() throws Exception {
      Optional.ofNullable(caManager.getCa(caName)).orElseThrow(
          () -> new CmdFailure("CA " + caName + " not available"));
      ...
      if (withBaseCrl) {
        ...
        byte[] extnValue = X509Util.getCoreExtValue(extns, OIDs.Extn.deltaCRLIndicator);
        if (extnValue != null) {
          if (baseCrlOut == null) {
