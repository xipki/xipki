...
public abstract class MacAuditService implements AuditService {
  ...
  protected String formatDate(Instant date) {
    return DTF.format(date.atZone(ZoneOffset.UTC));
  }
  ...
  private String buildMacPayload(
    ...
    return formatDate(date) + DELIM + levelText + DELIM + eventType
        + DELIM + shardId + DELIM + thisId + DELIM + previousId + INNER_DELIM
        + (previousTag == null ? "" : previousTag) + DELIM + message;
  }
  ...
  private String buildIntegrityText() {
    byte[] plaintext = StringUtil.toUtf8Bytes(VERSION_V1
        + DELIM + shardId + DELIM + id.get() + DELIM + previousTag);
    ...
    return VERSION_V1 + DELIM + keyId + DELIM + encoder.encodeToString(iv)
        + DELIM + encoder.encodeToString(cipherText);
  }
