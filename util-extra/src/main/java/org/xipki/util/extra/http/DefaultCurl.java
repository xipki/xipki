...
public class DefaultCurl implements Curl {
  ...
  private static final class HostConf {
    private final List<String> urlPattern;
    private final SslConf sslContext;

    public HostConf(List<String> urlPattern, SslConf sslContext) {
      this.urlPattern = Args.notNull(urlPattern, "urlPattern");
      this.sslContext = Args.notNull(sslContext, "sslContext");
    }
  }

  private static final class CurlConf {
    private final List<HostConf> hostConfs;

    public CurlConf(List<HostConf> hostConfs) {
      ...
      for (HostConf m : hostConfs) {
        for (String p : m.urlPattern) {
          if (urlPatterns.contains(p)) {
            throw new IllegalArgumentException("duplicated urlPattern " + p);
          }
          urlPatterns.add(p);
        }
      }
    ...
    private static CurlConf parse(File file)
      ...
      try {
        ...
        List<HostConf> hosts = null;
        JsonList hostsConf = root.getList("hostConfs");
        if (hostsConf != null) {
          hosts = new ArrayList<>(hostsConf.size());

          for (JsonMap hostConf : hostsConf.toMapList()) {
            JsonMap map = hostConf.getMap("sslContext");
