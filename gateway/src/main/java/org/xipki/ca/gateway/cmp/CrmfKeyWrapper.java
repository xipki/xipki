  ...
  static class ECIESAsymmetricKeyWrapper extends CrmfKeyWrapper {
    ...
    public ECIESAsymmetricKeyWrapper(PublicKey publicKey) {
      ...
      this.algorithmIdentifier = new AlgorithmIdentifier(
          OIDs.Secg.id_ecies_specifiedParameters, buildECIESParameters());
    }
    ...
    public byte[] generateWrappedKey(byte[] keyToWrap)
        throws OperatorException {
      try {
        BlockCipher cbcCipher = CBCBlockCipher.newInstance(
        ...
        // "Furthermore here the 16 octet or 128 bit IV for AES in CBC mode
        // should always take the value 0000000000000000_{16}"
        byte[] iv = new byte[16];
        IESParameterSpec spec = new IESParameterSpec(null, null,
            aesKeySize, aesKeySize, iv);
        cipher.engineInit(Cipher.ENCRYPT_MODE, publicKey, spec,
            new SecureRandom());
        ...
        System.arraycopy(bcResult, 0, ephemeralPublicKey,
            0, ephemeralPublicKeyLen);
        array[0] = new DEROctetString(ephemeralPublicKey);

        // symmetricCiphertext OCTET STRING
        int symmetricCiphertextLen =
            bcResult.length - ephemeralPublicKeyLen - macLen;
        byte[] symmetricCiphertext = new byte[symmetricCiphertextLen];
        System.arraycopy(bcResult, ephemeralPublicKeyLen, symmetricCiphertext,
            0, symmetricCiphertextLen);
        array[1] = new DEROctetString(symmetricCiphertext);

        // macTag OCTET STRING
        byte[] macTag = new byte[macLen];
        System.arraycopy(bcResult,
            ephemeralPublicKeyLen + symmetricCiphertextLen,
            macTag, 0, macLen);
    ...
    private ASN1Sequence buildECIESParameters() {
      ...
      // SymmetricEncryption
      AlgorithmIdentifier symmetricEncryption =
          new AlgorithmIdentifier(OIDs.Secg.id_aes128_cbc_in_ecies);
