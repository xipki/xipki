...
import static org.xipki.security.exception.ErrorCode.BAD_POP;
...
import static org.xipki.util.extra.audit.AuditLevel.ERROR;
import static org.xipki.util.extra.audit.AuditLevel.INFO;
...
public class RestResponder {
  ...
  private static final String CT_octetStream = "application/octet-stream";
  ...
  private static final String CMD_enroll_serverkeygen_twin =
      "enroll-serverkeygen-twin";

  private static final String CMD_crl = "crl";

  private static final String CMD_kem_encapkey = "kem-encapkey";
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(RestResponder.class);
  ...
  static {
    knownCommands = CollectionUtil.asUnmodifiableSet(
        CMD_cacert,             CMD_cacerts,
        CMD_dh_pop_certs,       CMD_revoke_cert,
        CMD_unsuspend_cert,     CMD_enroll_cert,
        CMD_enroll_cross_cert,  CMD_enroll_serverkeygen,
        CMD_enroll_cert_twin,   CMD_enroll_serverkeygen_twin,
        CMD_crl,                CMD_rekey_cert,
        CMD_rekey_serverkeygen, CMD_kem_encapkey);
  }
  ...
  public HttpResponse service(String path, byte[] request,
    ...
    try {
      ...
      if (tokens.length == 1) {
        ...
        CaProfileConf caProfileConf =
            caProfilesControl.getCaProfile("default");
      ...
      if (hdrValue != null && hdrValue.startsWith("Basic ")) {
        ...
        X509Cert clientCert = Optional.ofNullable(
            TlsHelper.getTlsClientCert(httpRetriever, reverseProxyMode))
      ...
      switch (command) {
        case CMD_enroll_cross_cert:
          respContent = enrollCrossCert(caName, certProfile, requestor,
                        request, httpRetriever, event);
          break;
        case CMD_enroll_cert:
        case CMD_enroll_serverkeygen:
        case CMD_enroll_cert_twin:
        ...
        case CMD_rekey_serverkeygen:
          respContent = enrollCerts(command, caName, certProfile, requestor,
                        request, httpRetriever, event);
          break;
        ...
        case CMD_unsuspend_cert:
          ...
          respContent = null;
          break;
        case CMD_kem_encapkey:
          respContent = generateKemEncapKey(request);
          break;
        default:
  ...
  private HttpRespContent enrollCerts(
    ...
    boolean twin = CMD_enroll_cert_twin.equals(command)
                    || CMD_enroll_serverkeygen_twin.equals(command);
    boolean caGenKeyPair = CMD_enroll_serverkeygen.equals(command)
                    || CMD_enroll_serverkeygen_twin.equals(command)
                    || CMD_rekey_serverkeygen.equals(command);
    boolean rekey = CMD_rekey_cert.equals(command)
                    || CMD_rekey_serverkeygen.equals(command);
    ...
    if (profile != null) {
      ...
      if (profileEnc != null
          && !requestor.isCertprofilePermitted(caName, profileEnc)) {
    ...
    if (caGenKeyPair) {
      if (ct.startsWith("text/plain")) {
        ...
        // The PKCS#10 will only be used for transport of subject and
        // extensions.
        // The associated key will not be used, so the verification of POP
        // is skipped.
    ...
    if (rekey) {
      ...
      byte[] caSha1;
      try {
        String strCertSerialNo =
            httpRetriever.getParameter(PARAM_oldcert_serial);

        if (StringUtil.isBlank(strCertSerialNo)) {
          throw new HttpRespAuditException(HttpStatusCode.SC_BAD_REQUEST,
              "required parameter " + PARAM_oldcert_serial + " not specified",
              INFO, FAILED);
        }
      ...
      OldCertInfo.BySha1FpAndSerial byFsn =
          new OldCertInfo.BySha1FpAndSerial(caSha1, certSerialNo);
    ...
    EnrollOrPollCertsResponse sdkResp = rekey
        ? sdk.reenrollCerts(caName, sdkReq)
  ...
  private HttpRespContent enrollCrossCert(
    ...
    if (!CT_pem_file.equalsIgnoreCase(ct)) {
      ...
      throw new HttpRespAuditException(
          HttpStatusCode.SC_UNSUPPORTED_MEDIA_TYPE, message, INFO, FAILED);
    }
    ...
    try (PemReader pemReader = new PemReader(
        new InputStreamReader(new ByteArrayInputStream(request)))) {
      while (true) {
    ...
    if (csrBytes == null) {
      throw new HttpRespAuditException(HttpStatusCode.SC_BAD_REQUEST,
          "PEM CSR is not specified", INFO, FAILED);
    }

    if (targetCertBytes == null) {
      throw new HttpRespAuditException(HttpStatusCode.SC_BAD_REQUEST,
          "PEM CERTIFICATE is not specified", INFO, FAILED);
    }
    ...
    try {
      ...
      throw new HttpRespAuditException(HttpStatusCode.SC_BAD_REQUEST,
          ex.getMessage(), INFO, FAILED);
    }

    SubjectPublicKeyInfo subjectPublicKeyInfo =
        targetCert.getSubjectPublicKeyInfo();
  ...
  private static void checkResponse(
    ...
    if (n != expectedSize) {
      throw new HttpRespAuditException(
          HttpStatusCode.SC_INTERNAL_SERVER_ERROR,
          "expected " + expectedSize + " cert, but received " + n,
          INFO, FAILED);
    }
  }
  ...
  private void unRevoke(
      String command, Requestor requestor, XiHttpRequest httpRetriever,
      AuditEvent event)
      throws OperationException, HttpRespAuditException,
      SdkErrorResponseException {
    boolean revoke = command.equals(CMD_revoke_cert);
    Requestor.Permission permission = revoke
        ? Requestor.Permission.REVOKE_CERT
        : Requestor.Permission.UNSUSPEND_CERT;
    ...
    if (!revoke) {
      UnsuspendOrRemoveCertsRequest sdkReq = new UnsuspendOrRemoveCertsRequest(
          caSha1, null, null, new BigInteger[]{serialNumber});
      ...
      String strInvalidityTime =
          httpRetriever.getParameter(PARAM_invalidity_time);
      if (StringUtil.isNotBlank(strInvalidityTime)) {
        invalidityTime = DateUtil.parseUtcTimeyyyyMMddhhmmss(strInvalidityTime);
      }

      RevokeCertsRequest.Entry entry =
          new RevokeCertsRequest.Entry(serialNumber, reason, invalidityTime);

      RevokeCertsRequest sdkReq = new RevokeCertsRequest(caSha1,
          null, null, new RevokeCertsRequest.Entry[]{entry});
  ...
  private HttpRespContent generateKemEncapKey(byte[] request)
    ...
    return HttpRespContent.ofOk(CT_octetStream, respBytes);
  }
