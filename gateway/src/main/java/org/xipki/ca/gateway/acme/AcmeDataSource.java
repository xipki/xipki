...
public class AcmeDataSource {
  ...
  private static final String SQL_ADD_ACCOUNT = "INSERT INTO ACCOUNT " +
      "(ID,LUPDATE,STATUS,JWK_SHA256,DATA) VALUES (?,?,?,?,?)";

  private static final String SQL_ADD_ORDER =
      "INSERT INTO ORDER2 (ID,LUPDATE,ACCOUNT_ID,STATUS,EXPIRES," +
      "CERT_NAFTER,CERT_SHA256,CERTREQ_META,CSR,CERT,AUTHZS) " +
          "VALUES (?,?,?,?,?,?,?,?,?,?,?)";

  private static final String SQL_DELETE_ORDER_CERT_EXPIRED =
  ...
  public AcmeDataSource(DataSourceWrapper dataSource) {
    ...
    this.sqlGetOrderById = dataSource.buildSelectFirstSql(1,
        "ACCOUNT_ID,STATUS,EXPIRES,CERTREQ_META,AUTHZS,CERT_SHA256 " +
            "FROM ORDER2 WHERE ID=?");
    this.sqlGetOrderByCertSha256 = dataSource.buildSelectFirstSql(1,
        "ID,ACCOUNT_ID,STATUS,EXPIRES,CERTREQ_META,AUTHZS " +
            "FROM ORDER2 WHERE CERT_SHA256=?");
  }
  ...
  private AcmeAccount getAccount(PreparedStatement ps, String sql)
    ...
    try {
      ...
      AcmeAccount.Data data = AcmeAccount.Data.parse(
          JsonParser.parseMap(rs.getString("DATA"), false));
  ...
  public void addNewOrder(AcmeOrder order) throws AcmeSystemException {
    ...
    PreparedStatement ps = prepareStatement(sql);
    // ID,LUPDATE,ACCOUNT,STATUS,EXPIRES,CERT_NAFTER,CERT_SHA256,
    // CERTREQ_META,CSR,CERT,AUTHZS
    try {
      order.updateStatus();
      ...
      if (certBytes == null) {
        ...
        ps.setString(index, encoder.encodeToString(certBytes));
      }
      index++;
  ...
  private AcmeOrder buildOrder(ResultSet rs, Long id)
    ...
    try {
      ...
      throw new AcmeSystemException(null, ex);
    } catch (CodecException ex) {
      throw new AcmeSystemException(ex);
    }
  }
  ...
  public List<ChallId> getChallengesToValidate() throws AcmeSystemException {
    final String sql = dataSource.buildSelectFirstSql(1000,
        "ID,AUTHZS FROM ORDER2 WHERE STATUS=?");
    PreparedStatement ps = prepareStatement(sql);
    ResultSet rs = null;
    try {
  ...
  public int cleanOrders(Instant certNotAfter, Instant notFinishedOrderExpires)
    ...
    if (notFinishedOrderExpires.isAfter(dateLimit)) {
      throw new IllegalArgumentException("notFinishedOrderExpires "
          + notFinishedOrderExpires + " is not allowed");
    }
  ...
  private static String toJson(JsonEncodable encodable) {
    return JsonBuilder.toJson(encodable.toCodec());
  }
