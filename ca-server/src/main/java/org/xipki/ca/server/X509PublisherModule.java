...
class X509PublisherModule extends X509CaModule {
  ...
  int publishCert(CertificateInfo certInfo, boolean saveKeypair) {
    ...
    for (IdentifiedCertPublisher publisher : publishers()) {
      ...
      if (!successful) {
        ...
        failedPublishers.add(publisher.ident().name());
      }
    } // end for
    ...
    if (LOG.isWarnEnabled()) {
      LOG.warn("could not publish to publishers {}: {}", failedPublishers,
          Base64.encodeToString(certInfo.cert().cert().getEncoded(), true));
    }
    return 2;
  } // method publishCert0
  ...
  boolean republishCerts(List<String> publisherNames, int numThreads) {
      ...
      for (String publisherName : publisherNames) {
        ...
        if (publisher == null) {
          throw new IllegalArgumentException("could not find publisher "
              + publisherName + " for CA " + caIdent.name());
        }
    ...
    for (IdentifiedCertPublisher publisher : publishers) {
      if (publisher.publishesGoodCert()) {
      ...
      for (IdentifiedCertPublisher publisher : publishers) {
        ...
        if (!successful) {
          LOG.error("republish CA certificate {} to publisher {} failed",
              caIdent.name(), publisher.ident().name());
          return false;
        }
      }
  ...
  void publishCrl(X509Crl crl) {
    try {
      ...
      LOG.error("could not add CRL ca={}, thisUpdate={}: {}, ",
          caIdent.name(), crl.thisUpdate(), ex.getMessage());
  ...
  boolean publishCertRemoved(CertWithDbId certToRemove) {
    ...
    for (IdentifiedCertPublisher publisher : publishers()) {
      ...
      successful = false;
      X509Cert cert = certToRemove.cert();
      if (LOG.isErrorEnabled()) {
        LOG.error(
            "removing certificate issuer='{}', serial={}, subject='{}' from publisher {} failed.",
            cert.issuerText(), cert.serialNumberHex(), cert.subjectText(), publisher.ident());
      }
    } // end for
  ...
  void publishCertRevoked(CertWithRevocationInfo revokedCert) {
    for (IdentifiedCertPublisher publisher : publishers()) {
      ...
      if (!successful) {
        LOG.error("could not publish revocation of certificate to the publisher {}",
            publisher.ident());
      }
    } // end for
  }
  ...
  void publishCertUnrevoked(CertWithDbId unrevokedCert) {
    ...
    LOG.error("could not publishCertUnrevoked of certificate {} to publishers {}",
        unrevokedCert.certId(), failedPublishers);
  }
  ...
  private List<IdentifiedCertPublisher> publishers() {
    return caManager.getIdentifiedPublishersForCa(caIdent.name());
  }
