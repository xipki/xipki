...
public abstract class DbPortWorker extends DbWorker {
  ...
  public DbPortWorker(DataSourceFactory datasourceFactory, String dbConfFile,
                      char[] password)
  ...
  private static ZipParameters getZipParameters() {
    ...
    zipParameters.setFileComment("Database exported by "
        + StringUtil.getBundleNameVersion(DbPortWorker.class));
  ...
  protected void encrypt(File dir) throws IOException {
    ...
    try (ZipFile zipFile =
             new ZipFile(new File(zipDir, "main.zip"), password)) {
      // split length: 64M Byte
      zipFile.createSplitZipFileFromFolder(dir, getZipParameters(),
          true, 64L * 1024 * 1024);
    }
  ...
  public static class ImportCaDb extends DbPortWorker {
    ...
    public ImportCaDb(DataSourceFactory datasourceFactory, String caConfDbFile,
                      String caDbFile, boolean resume, String srcFolder,
      ...
      this.caDataSource = datasourceFactory.createDataSource(
          "ds-" + caDbFile, props);
    }
    ...
    protected void run0() throws Exception {
      File processLogFile = new File(srcFolder,
      ...
      try {
        ...
        CaconfDbImporter caConfImporter =
            new CaconfDbImporter(datasource, srcFolder, stopMe);
        if (!resume) {
          // CAConfiguration
  ...
  public static class ExportCaDb extends ExportCaCertStoreDb {
    ...
    public ExportCaDb(
      ...
      if (caConfDbFile != null) {
        ...
        this.caConfSource = datasourceFactory.createDataSource(
            "ds-" + caConfDbFile, props);
      } else {
      ...
      try {
        if (!resume) {
          ...
          CaconfDbExporter caConfExporter = new CaconfDbExporter(
              caConfSource, destFolder, stopMe);
        ...
        CaCertstoreDbExporter certStoreExporter = new CaCertstoreDbExporter(
            datasource, destFolder, numCertsInBundle, numCertsPerSelect,
            resume, stopMe);
  ...
  public static class ExportCaCertStoreDb extends DbPortWorker {
    ...
    public ExportCaCertStoreDb(
        ...
        String destFolder, boolean resume, int numCertsInBundle,
    ...
    protected void checkDestFolder() throws IOException {
      ...
      File processLogFile = new File(destFolder,
  ...
  public static class ImportOcspFromCaDb extends DbPortWorker {
    ...
    public ImportOcspFromCaDb(
        DataSourceFactory datasourceFactory, String dbConfFile,
        String publisherName, boolean resume, String srcFolder,
        int batchEntriesPerCommit, char[] password)
        throws InvalidConfException, IOException {
      super(datasourceFactory, dbConfFile, password);
    ...
    protected void run0() throws Exception {
      ...
      try {
        OcspCertStoreFromCaDbImporter certStoreImporter =
            new OcspCertStoreFromCaDbImporter(
  ...
  private static void printFinishedIn(long startMs) {
    ...
    System.out.println("Finished in " +
        StringUtil.formatTime(duration, false));
  }
