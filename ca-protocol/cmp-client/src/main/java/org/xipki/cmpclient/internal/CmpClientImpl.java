...
public final class CmpClientImpl implements CmpClient {
  ...
  public synchronized boolean init() {
    ...
    try {
      sslSocketFactory = sslCc.getSslSocketFactory();
      hostnameVerifier = sslCc.buildHostnameVerifier();
    } catch (ObjectCreationException ex) {
      LOG.error("could not initialize SSL configuration " + conf.getSsl().getName()
          + ": " + ex.getMessage(), ex);
      return false;
    }
    ...
    if (sigResponderConf != null) {
      X509Cert cert;
      try {
        cert = X509Util.parseCert(sigResponderConf.getCert().readContent());
      } catch (CertificateException | IOException ex) {
        LogUtil.error(LOG, ex, "could not configure responder");
        return false;
      }
      ...
      signatureResponder = new Responder.SignatureCmpResponder(cert, sigAlgoValidator);
    }
    ...
    if (responderConf.getDhPopCerts() != null) {
      ...
      for (FileOrBinary m : responderConf.getDhPopCerts()) {
        X509Cert cert;
        try {
          cert = X509Util.parseCert(m.readContent());
        } catch (IOException | CertificateException ex) {
          LogUtil.error(LOG, ex, "could not configure DhPocCerts");
          return false;
        }
