...
import org.xipki.scep4j.client.exception.OperationNotSupportedException;
...
import org.xipki.scep4j.message.DecodedNextCAMessage;
...
public abstract class Client
{
    ...
    throws ScepClientException
    {
        ...
        {
            if (httpGetOnly == false && cACaps.containsCapability(CACapability.POSTPKIOperation))
            {
    ...
    throws ScepClientException
    {
        // getCACaps
        {
            ScepHttpResponse resp = httpSend(Operation.GetCACaps);
            this.cACaps = CACaps.getInstance(new String(resp.getContentBytes()));
        }

        // getCACert
        {
            ScepHttpResponse resp = httpSend(Operation.GetCACert);
            this.authorityCertStore = retrieveCACertStore(resp, cACertValidator);
    ...
    throws ScepClientException
    {
        ...
        // draft-nourse-scep
        if (isGutmannScep() == false)
        {
            return scepPkcsReq(csr, identityKey, identityCert);
        }

        // draft-gutmann-scep
        if (ScepUtil.isSelfSigned(identityCert) == false)
        {
    ...
    throws ScepClientException
    {
        ...
        if (ScepConstants.CT_x_x509_next_ca_cert.equalsIgnoreCase(ct) == false)
        {
            throw new ScepClientException("invalid Content-Type '" + ct + "'");
        }
        ...
        if (resp.getSignatureCert().equals(authorityCertStore.getSignatureCert()) == false)
        {
