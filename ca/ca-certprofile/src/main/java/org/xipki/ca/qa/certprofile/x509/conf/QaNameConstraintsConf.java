...
public class QaNameConstraintsConf extends QaExtensionConf
{
    private final List<QaGeneralSubtreeConf> permittedSubtrees;
    private final List<QaGeneralSubtreeConf> excludedSubtrees;

    public QaNameConstraintsConf(NameConstraints jaxb)
    {
        super(jaxb.getCondition());

        if(jaxb.getPermittedSubtrees() != null && jaxb.getPermittedSubtrees().getBase().isEmpty() == false)
        {
            List<QaGeneralSubtreeConf> list = new LinkedList<>();
            List<GeneralSubtreeBaseType> bases =  jaxb.getPermittedSubtrees().getBase();
            for(GeneralSubtreeBaseType base : bases)
            {
                list.add(new QaGeneralSubtreeConf(base));
            }
    ...
    public List<QaGeneralSubtreeConf> getPermittedSubtrees()
    {
        return permittedSubtrees;
    }

    public List<QaGeneralSubtreeConf> getExcludedSubtrees()
    {
